...
namespace vamp::planning
{
    ...
    struct CRRTC
    {
        ...
        {
            return solve(start, std::vector<Configuration>{goal}, environment, crrtc_settings, constraint, rng);
        }
            ...
            typename RNG::Ptr rng) noexcept -> PlanningResult<Robot>
        {
            ...
            for (const auto &goal : goals)
            {
                ...
                if (vamp::planning::constraint::project_constraint_motion<Robot, rake, resolution>(
                        ...
                        constraint,
                        environment,
                        static_cast<vamp::planning::constraint::ProjMethod>(crrtc_settings.constraint_settings.projection_method),
            ...
            int dyndomfailed = 0;
            while (iter++ < crrtc_settings.rrtc_settings.max_iterations and free_index < crrtc_settings.rrtc_settings.max_samples and not connected)
            {
                // if (iter % 1 == 0)
                ...
                if ((not crrtc_settings.rrtc_settings.balance) or ratio < crrtc_settings.rrtc_settings.tree_ratio)
                {
                ...
                auto extension_vector =
                    (reach) ? nearest_vector : nearest_vector * (crrtc_settings.rrtc_settings.range / nearest_distance);
...

                {
                    ...
                    {
                        if (free_index >= crrtc_settings.rrtc_settings.max_samples) break;  // Prevent buffer overflow
                        ...
                        if (crrtc_settings.rrtc_settings.dynamic_domain and nearest_radius != std::numeric_limits<float>::max())
                        {
                    ...
                    const std::size_t n_extensions = static_cast<std::size_t>(std::ceil(other_nearest_distance / crrtc_settings.rrtc_settings.range));
                    ...
                    while (not connected)
                    {
                        ...
                        auto other_extension_vector =
                            (other_reach) ? other_nearest_vector : other_nearest_vector * (crrtc_settings.rrtc_settings.range / other_nearest_distance);

                        if(not vamp::planning::constraint::project_constraint_vector<Robot, rake, resolution>(
                            prior,
                            other_extension_vector,
                            (other_reach) ? other_nearest_distance : crrtc_settings.rrtc_settings.range,
                            projected_vector,
                            constraint,
                            environment,
                            static_cast<vamp::planning::constraint::ProjMethod>(crrtc_settings.constraint_settings.projection_method),
                ...
                else if (crrtc_settings.rrtc_settings.dynamic_domain)
                {
                    ...
                    else
                    {
                        radii[nearest_node.index] =
                            std::max(radii[nearest_node.index] * (1.F - crrtc_settings.rrtc_settings.alpha), crrtc_settings.rrtc_settings.min_radius);
                    }
                    validate_failed++;
                }
            }
