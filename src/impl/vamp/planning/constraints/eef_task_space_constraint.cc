...
namespace vamp::planning {
    ...
    class TaskSpaceConstraint {
        ...
        public:
            ...
            // compute_gradient(q) - calls distanceToConstraint(q) and then calls Robot::compute_gradient(j_e_array)
            // projectStep(q) - calls grad = compute_gradient(q) and then calls integrate_gradient(q, grad, alpha)
            //
            inline auto distanceToConstraint(const ConfigurationBlock& q_block)
            {
                ...
                for (std::size_t i = 0; i < error_size; ++i)
                {
                    ...
                    j_e_array.e[i] =
                        (j_e_array.e[i] - tsr_inp[lbi]).min(0.F) +
                        (j_e_array.e[i] - tsr_inp[ubi]).max(0.F);
