...
namespace vamp::planning
{
    ...
    protected:
        using ConfigurationBlock = typename Robot::ConfigurationBlock<rake>;
        std::pair<std::array<float, 6>, std::array<float, 6>> bimanual_bounds;  // error bounds in se3
        Eigen::Transform<float, 3, Eigen::Isometry> right_eef_pose_w_ref_left_eef;

        std::pair<std::array<float, 6 * Robot::n_eef>, std::array<float, 6 * Robot::n_eef>> eef_pos_bounds;  // error bounds in se3
        std::array<Eigen::Transform<float, 3, Eigen::Isometry>, Robot::n_eef> eef_pose_w_ref_reference;
        std::array<Eigen::Transform<float, 3, Eigen::Isometry>, Robot::n_eef> ref_frame_w_world;
        ...
        {
            vamp::FloatVector<rake, (2 + 6 + 6 * 4) * Robot::dimension> J;  // jacobian
            vamp::FloatVector<rake, (2 + 6 + 6 * 4)> err;                   // error vector

            auto &operator[](size_t index)
            {
                if (index < (2 + 6 + 6 * 4) * Robot::dimension)
                {
                    return J[index];
                }
                else if (
                    index >= (2 + 6 + 6 * 4) *  Robot::dimension &&
                    index < (2 + 6 + 6 * 4) * Robot::dimension + (2 + 6 + 6 * 4))
                {
                    return err[index - (2 + 6 + 6 * 4) * Robot::dimension];
                }
                else
                {
                    return err[0];
            ...
            JacobianProjectInp &
            operator=(vamp::FloatVector<rake, (2 + 6 + 6 * 4) + (2 + 6 + 6 * 4) * Robot::dimension> y)
            {
                for (size_t i = 0; i < (2 + 6 + 6 * 4); i++)
                {
                    err[i] = y[(2 + 6 + 6 * 4) * Robot::dimension + i];
                }
                for (size_t i = 0; i < (2 + 6 + 6 * 4) * Robot::dimension; i++)
                {
                    J[i] = y[i];
                }
                return *this;
    ...
    public:
        BimanualCoMTSRTaskSpaceConstraint(
          ...
          , bimanual_bounds(bimanual_bounds)
        {
        ...
        vamp::FloatVector<rake, 1> distanceToConstraint(const ConfigurationBlock &q)
        {
            ...
            // for (size_t i = 0; i < 6 * Robot::dimension + 6; i++)
            //     std::cout << tsr_err_jac[{i, 0}] << ", ";
            // std::cout << std::endl;


            // for (size_t i = 0; i < Robot::dimension; i++)
            //     std::cout << tsr_function_inp.q[{i, 0}] << ", ";
            // std::cout << std::endl;
            // for (size_t i = 0; i < 7; i++)
            //     std::cout << tsr_function_inp.rTlB[{i, 0}] << ", ";
            // std::cout << std::endl;

            // for (size_t i = 0; i < 6; i++)
            //     std::cout << tsr_function_inp.lbB[{i, 0}] << ", ";
            // std::cout << std::endl;
            // for (size_t i = 0; i < 6; i++)
            //     std::cout << tsr_function_inp.ubB[{i, 0}] << ", ";
            // std::cout << std::endl;

            // std::cout << " -- Full input --" << std::endl;
            // for (size_t i = 0; i < (7 + 6 + 6) + Robot::dimension; i++)
            //     std::cout << i << " : " << tsr_function_inp[i] << std::endl;
            // std::cout << std::endl;
            ...
            for (size_t i = 0; i < 6 * Robot::n_eef * Robot::dimension; i++)
                jac_proj_inp[8 * Robot::dimension +i] = eef_pos_tsr_err_jac[i];

            for (size_t i = 0; i < 2; i++)
                jac_proj_inp[i + (2 + 6 + 6 * Robot::n_eef) * Robot::dimension] = com_err_jac[2 * Robot::dimension + i];

            for (size_t i = 0; i < 6; i++)
                jac_proj_inp[i + (2 + 6 + 6 * Robot::n_eef) * Robot::dimension + 2] = tsr_err_jac[i + 6 * Robot::dimension];

            for (size_t i = 0; i < 6 * Robot::n_eef; i++)
                jac_proj_inp[i + (2 + 6 + 6 * Robot::n_eef) * Robot::dimension + 2 + 6] = eef_pos_tsr_err_jac[i + 6 * Robot::dimension * Robot::n_eef];
            // std::cout << "Error : ";
            // for(auto i=0U; i < 2 + 6 + 6 * Robot::n_eef; i++){
            //     std::cout << std::setprecision(5) << jac_proj_inp.err[{i, 0}] << " ";
            // }
            // std::cout << std::endl;

            const size_t jac_offset = (2 + 6 + 6 * Robot::n_eef) * Robot::dimension + 2 + 6;
            for (size_t i = 0; i < 6 * Robot::n_eef; i++)
            {
                jac_proj_inp[i + jac_offset] =
                    (jac_proj_inp[i + jac_offset] - eef_pos_tsr_function_inp.lbB[i]).min(0.F) +
                    (jac_proj_inp[i + jac_offset] - eef_pos_tsr_function_inp.ubB[i]).max(0.F);
            }
            ...
            for (size_t i = 1; i < 2 + 6 + 6 * Robot::n_eef; i++)
            {
