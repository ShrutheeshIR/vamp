...
// {-0.154247,-1.38187,1.88281,-1.42206,-0.691598,3.52511,1.43219};

// static constexpr Robot::ConfigurationArray start = {-1.314, 1.339, 1.095, -2.495, 0.513, 2.551,
// -1.495, 1.294, 1.309, -1.055, -2.493, -0.713, 2.500, -3.014 }; static constexpr Robot::ConfigurationArray
// goal = {-1.068, 0.602, 1.329, -1.797, 1.059, 1.959, -1.149, 1.033, 0.446, -1.224, -1.837,
// -1.286, 1.917, 2.813};

static constexpr Robot::ConfigurationArray start =
    {-1.45508, -0.718918, 2.46299, -0.29207, -0.396585, 2.28344, -0.613603};
static constexpr Robot::ConfigurationArray goal =
    {0.2013, 0.0540317, -2.7026, -0.138382, -0.902814, 3.35919, -1.78867};
...
auto main(int, char **) -> int
{
    ...
    // lTr_start.translation().transpose() << std::endl;

    // auto lTr_goal = goal_eefk[0].inverse() * goal_eefk[1];
    // Eigen::Quaternionf q2(lTr_goal.linear());
    // std::cout << q2.w() << ", " << q2.x() << ", " << q2.y() << ", " << q2.z() << ", " <<
    // lTr_goal.translation().transpose() << std::endl;
    ...
    for (auto i = 0U; i < rake - 1; i++)
    {
        ...
        {
            // std::cout << i << " " << j << " " << initial_projected_block[{j, i+1}] << ", " <<
            // initial_projected_block[{j, i}] << " " << initial_projected_block[{j, i+1}] -
            // initial_projected_block[{j, i}] << std::endl;
            inter_distance =
                inter_distance +
                std::pow(initial_projected_block[{j, i + 1}] - initial_projected_block[{j, i}], 2);
        }
        std::cout << "Distance between points " << i << " and " << i + 1 << ": " << std::sqrt(inter_distance)
                  << " " << (distance / rake) << std::endl;
    ...
    // // auto diff_block = configuration_block_difference(initial_projected_block,
    ...
    auto lane_dist_block = vamp::planning::constraint::inter_lane_distance_block(
        initial_projected_block, Robot::Configuration(start));
    ...
    for (auto i = 0U; i < rake; i++)
    {
        ...
        for (auto j = 0U; j < Robot::dimension; j++)
        {
            ...
            {
                std::cout << "Error at " << i << ", " << j << ": " << diff_arr[i + j * rake]
                          << " != " << lane_dist_block[{j, i}] << std::endl;
            }
    ...
    std::cout << std::endl;
    std::cout << "Inter Rake Distance: " << inter_rake_distance << " , "
              << std::sqrt(inter_rake_distance.hmax()) << ", " << max_inter_dist << std::endl;

    // typename Robot::template ConfigurationBlock<rake> shifted_block = typename Robot::template
    // ConfigurationBlock<rake>(diff_arr);
    ...
    // for(auto& proj_vec : projected_vector2){
    //     ret = vamp::planning::constraint::project_constraint_motion<Robot, rake,
    //     Robot::resolution>(proj_vec, proj_vec, projected_vector_inside, task_constraint, env_v); std::cout
    //     << "Projected Motion: " << ret << " --
    //     ";
    ...
    // Robot::Configuration start_plus_percent = {-1.289,  1.039,  1.079, -2.12 ,  0.553,  2.082,
    // -1.181,  1.165, 1.11 , -1.048, -2.128, -0.5  ,  2.138, -0.202}; Robot::ConfigurationArray test =
    // {-1.289,  1.039,  1.079, -2.12 ,  0.553,  2.082, -1.181,  1.165, 1.11 , -1.048, -2.128, -0.5  ,  2.138,
    // -0.202};
    ...
    // vamp::planning::constraint::ProjMethod::InnerLM, 10.0, 1.0); std::cout << std::endl; first = true; for
    // (auto i = 0U; i < Robot::dimension; ++i){
    //     holder[i] = projected_block[{i, 0}];
    //     std::cout << holder[i] << ", ";
    //     if (!first) outfile << ",";
    //     outfile << holder[i];
