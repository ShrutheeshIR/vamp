...
struct VAMPStateValidator : public ob::StateValidityChecker
{
    ...
    auto isValid(const ob::State *state) const -> bool override
    {
        ...
        bool projection_result =
            vamp::planning::constraint::project_constraint_motion<Robot, rake, Robot::resolution>(
                robot_config,
                robot_config,
                projected_vector,
                task_constraint,
...
int main()
{
    ...
    std::array<float, 6 * Robot::n_eef> tsr_lower_bound = {-10.01, -10.01, -0.01, -0.01, -0.01, -10.01};

    std::array<float, 6 * Robot::n_eef> tsr_upper_bound = {10.01, 10.01, 0.01, 0.01, 0.01, 10.01};
