...
std::string StripJsonComments(std::string_view in) {
  ...
  enum class State { kNormal, kInString, kInLineComment, kInBlockComment };
...
bool ParseDoubleArray(const nlohmann::json& j, std::vector<double>& out, std::string* err, const char* field_name) {
  if (!j.is_array()) {
    if (err) {
...
bool ParseSpheres(const nlohmann::json& spheres_obj, std::vector<Sphere>& out, std::string* err) {
  ...
  if (!spheres_obj.is_object()) {
    if (err) *err = "env.spheres must be an object mapping id -> [x, y, z, r].";
    return false;
  }
...
bool ParseCuboids(const nlohmann::json& cuboids_obj, std::vector<Cuboid>& out, std::string* err) {
  ...
  if (!cuboids_obj.is_object()) {
    if (err) *err = "env.cuboids must be an object mapping id -> [x, y, z, yaw, pitch, roll, dx, dy, dz].";
    return false;
  }
  ...
  for (auto it = cuboids_obj.begin(); it != cuboids_obj.end(); ++it) {
    ...
    if (!arr.is_array() || arr.size() != 9) {
      if (err) {
        std::ostringstream oss;
        oss << "env.cuboids['" << id << "'] must be an array of length 9: [x,y,z,yaw,pitch,roll,dx,dy,dz].";
        *err = oss.str();
      }
      return false;
    }
    ...
    for (int i = 0; i < 9; ++i) {
      ...
      switch (i) {
        ...
        case 1: c.y = v; break;
        case 2: c.z = v; break;
        case 3: c.yaw = v; break;
        case 4: c.pitch = v; break;
        case 5: c.roll = v; break;
        case 6: c.dx = v; break;
        case 7: c.dy = v; break;
        case 8: c.dz = v; break;
      }
...
bool ParseConfigFromJson(std::string_view json_text,
                         Config& out,
  ...
  if (!j.is_object()) {
    if (error_message) *error_message = "Top-level JSON must be an object.";
    return false;
  }

  // constraint
  if (!j.contains("constraint") || !j["constraint"].is_object()) {
    if (error_message) *error_message = "Missing or invalid 'constraint' object.";
    return false;
  }
  const auto& jc = j["constraint"];
  if (!jc.contains("constraint_type") || !jc["constraint_type"].is_string()) {
    if (error_message) *error_message = "constraint.constraint_type must be a string.";
    return false;
  }
  out.constraint.constraint_type = jc["constraint_type"].get<std::string>();

  if (!jc.contains("lower_bound") || !jc.contains("upper_bound")) {
    if (error_message) *error_message = "constraint.lower_bound and constraint.upper_bound are required.";
    return false;
  }
  {
    std::string err;
    if (!ParseDoubleArray(jc["lower_bound"], out.constraint.lower_bound, &err, "constraint.lower_bound")) {
      if (error_message) *error_message = err;
      return false;
    }
    if (!ParseDoubleArray(jc["upper_bound"], out.constraint.upper_bound, &err, "constraint.upper_bound")) {
      if (error_message) *error_message = err;
      return false;
    }
  }
  ...
  if (!j.contains("problem") || !j["problem"].is_object()) {
    if (error_message) *error_message = "Missing or invalid 'problem' object.";
    return false;
  }
  const auto& jp = j["problem"];
  if (!jp.contains("start") || !jp.contains("goal")) {
    if (error_message) *error_message = "problem.start and problem.goal are required.";
    return false;
  }
  {
    std::string err;
  ...
  if (!j.contains("env") || !j["env"].is_object()) {
    if (error_message) *error_message = "Missing or invalid 'env' object.";
    return false;
  }
  const auto& je = j["env"];
  {
...
bool ParseConfigFromFile(const std::string& file_path,
                         Config& out,
                         std::string* error_message,
                         const ParseOptions& options) {
...
bool ValidateConfig(const Config& cfg, std::string* error_message) {
  ...
  if (lb.size() != ub.size()) {
    if (error_message) *error_message = "constraint.lower_bound and constraint.upper_bound must have the same size.";
    return false;
  }
  if (lb.empty()) {
    if (error_message) *error_message = "constraint bounds must not be empty.";
    return false;
  }
  const size_t dim = lb.size();

  if (cfg.problem.start.size() != cfg.problem.goal.size()) {
    if (error_message) *error_message = "problem.start and problem.goal must have the same size.";
    return false;
  }
  if (cfg.problem.start.size() != dim) {
    if (error_message) {
      std::ostringstream oss;
      oss << "problem.start/problem.goal must have size " << dim << " to match constraint bounds.";
      *error_message = oss.str();
    }
    return false;
  }
