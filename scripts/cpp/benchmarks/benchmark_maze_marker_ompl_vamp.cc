...
public:
    ...
    bool project (Eigen::Ref< Eigen::VectorXd > x) const override
    {
        ...
        bool result = constraints.projectConfiguration(config_block, last_projected_block, vamp::planning::ProjMethod::OuterLM, 5.0, 0.75, 25, false);
        if (result){
            for (auto i = 0U; i < Robot::dimension; ++i) {
...
struct VAMPStateValidator : public ob::StateValidityChecker
{
    ...
    auto isValid(const ob::State *state) const -> bool override
    {
        ...
        bool projection_result = vamp::planning::project_constraint_motion<Robot, rake, Robot::resolution>(robot_config, robot_config, projected_vector, task_constraint, env_v, vamp::planning::ProjMethod::OuterLM, 0.75, 20, false);
