...
class SE3Constraint : public ob::Constraint
{
    ...
    SE3Constraint(const std::string& urdf_path, 
        ...
        tsr_(tsr)
        {
            ...
            // 2. Get the Frame ID
            eeFrame_ = model_ptr_->getFrameId("panda_tip");
...
auto run_rrtc(const Problem &problem) {
        ...
        std::string urdf_file = "/src/myfork/vamp/resources/panda_marker/panda_expo_spherized.urdf";
...
auto main(int argc, char **) -> int
{
    ...
    auto se3 = SE3Constraint("/src/myfork/vamp/resources/panda_marker/panda_expo_spherized.urdf", tsr);

    Eigen::VectorXd config(7);
    config << 0.99,-1.3,-0.65,-2.44,-0.59,1.69,2.13;
    Eigen::VectorXd out(6);
    se3.function(config, out);
    std::cout << "Constraint function output: " << out.transpose() << std::endl;

    // try to project
    // Eigen::VectorXd q_proj = q;
    auto projected = se3.project(config);
