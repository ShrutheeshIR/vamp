...
auto main(int, char **) -> int
{
    ...
    std::array<float, 6> lower_bound = {
        -0.01, -10.01, -0.03, -0.02, -0.01, -3.14
    };
    std::array<float, 6> upper_bound = {
        0.03, 10.01, 0.03, 0.02, 0.01, 3.14
    };
    ...
    auto startfk = Robot::eefk(start);
    Eigen::Quaternionf quaternions(startfk.linear());

    auto goalfk = Robot::eefk(goal);
    Eigen::Quaternionf quaterniong(goalfk.linear());

    std::cout << startfk.translation().transpose() << " " << quaternions << std::endl;
    std::cout << goalfk.translation().transpose() << quaterniong << std::endl;
