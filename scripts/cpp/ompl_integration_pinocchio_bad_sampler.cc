...
class SE3Constraint : public ob::Constraint
{
    ...
    SE3Constraint(const std::string& urdf_path, 
        ...
        void goodfunction(const Eigen::Ref<const Eigen::VectorXd> &q,
        ...
        bool project (Eigen::Ref< Eigen::VectorXd > x) const override
        {
            ...
            // std::cout << "Project called with x: " << x.transpose() << std::endl;
...
struct VAMPStateValidator : public ob::StateValidityChecker
{
    ...
    auto isValid(const ob::State *state) const -> bool override
    {
        ...
        std::cout << std::endl;
        // project and check if satisfied
        bool projection_success = constraint_->project(x);
        if (!projection_success) {
            // std::cout << "State is invalid, projection failed" << std::endl;
            return false;
        }
        ...
        // else overwrite *state with x

        // now print *state
        const Eigen::Map<Eigen::VectorXd> &y = *state->as<ob::ConstrainedStateSpace::StateType>();
